...
void efi_plat_init(EFI_SYSTEM_TABLE *st)
{
    ...
    debug_print(L"ST: %lX BS: %lX LP: %lX\n", st, st->BootServices, st->BootServices->LocateProtocol);

    EFI_STATUS status = uefi_call_wrapper(st->BootServices->LocateProtocol, 3, &MP_GUID, NULL, &mp);
    die_on(status != EFI_SUCCESS, L"Could not locate MP services protocol.");
}
...
void efi_plat_run_all_processors(fn_callback_routine routine, void *opaque)
{
    ...
    die_on(status, L"Unable to retrieve number of processors, status %d", status);
    ...
    if (enabled_procs > 1) {
        ...
        die_on(status, L"Unable to run routine on all processors, status %d", status);
    }
}
...
void efi_plat_processor_index(size_t *index)
{
    ...
    die_on(status, L"Unable to retrieve processor index, status %d", status);
}